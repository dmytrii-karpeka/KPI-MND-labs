...
# Лабораторна робота №5 "ПРОВЕДЕННЯ ТРЬОХФАКТОРНОГО ЕКСПЕРИМЕНТУ ПРИ ВИКОРИСТАННІ
# РІВНЯННЯ РЕГРЕСІЇ З УРАХУВАННЯМ КВАДРАТИЧНИХ ЧЛЕНІВ (ЦЕНТРАЛЬНИЙ ОРТОГОНАЛЬНИЙ КОМПОЗИЦІЙНИЙ ПЛАН)" з предмету МОПЕ
...
natur_x = [ [min_x[0], min_x[1], min_x[2]],
            ...
            [round(-1.215*delta_xi[0]+x0i[0], 3), x0i[1], x0i[2]],
            [round(1.215*delta_xi[0]+x0i[0], 3), x0i[1], x0i[2]],
            [x0i[0], round(-1.215*delta_xi[1]+x0i[1], 3), x0i[2]],
            [x0i[0], round(1.215*delta_xi[1]+x0i[1], 3), x0i[2]],
            [x0i[0], x0i[1], round(-1.215*delta_xi[2]+x0i[2], 3)],
            [x0i[0], x0i[1], round(1.215*delta_xi[2]+x0i[2], 3)],
            [x0i[0], x0i[1], x0i[2]]]

def experiment(m=3, n=15):
    ...
    def student_criteria(S_y, d):
        ...
        list_t = [bettaList[i] * S for i in range(n-4)]
